...
class OAuthService extends \Bkash\Service\AbstractService
{
    /**
     * Sends a request to Bkash's Connect API.
     *
     ...
     *
     * @return \Bkash\BkashObject the object returned by Bkash's Connect API
     */
    ...
    /**
     * Generates a URL to Bkash's OAuth form.
     *
     * @param null|array $params
     * @param null|array $opts
     *
     * @return string the URL to Bkash's OAuth form
     */
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \Bkash\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \Bkash\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \Bkash\BkashClient([clientId => <CLIENT-ID>
                ])`)".  You can find your client_ids '
              . 'in your Bkash dashboard at '
              . 'https://dashboard.bkash.com/account/applications/settings, '
              . 'after registering your account as a platform. See '
              . 'https://bkash.com/docs/connect/standard-accounts for details, '
              . 'or email support@bkash.com if you have any questions.';
     ...
     *
     * @return \Bkash\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \Bkash\Service\OAuthService from \Bkash\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \Bkash\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \Bkash\Util\RequestOptions::parse($opts);
    }

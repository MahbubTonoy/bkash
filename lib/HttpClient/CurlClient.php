...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    /** @var \Bkash\Util\RandomGenerator */
     ...
     * @param null|\Bkash\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\Bkash\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method) && (Bkash::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = Bkash::getCABundlePath();
        if (!Bkash::getVerifySslCerts()) {
        ...
        // force IPv4 resolving as Bkash's API servers are only accessible over
        // IPv4 (see. https://github.com/bkash/bkash-php/issues/1045).
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Bkash ({$url}).  Please check your "
                 . 'internet connection and try again.  If this problem persists, '
                 . "you should check Bkash's service status at "
                 . 'https://twitter.com/bkashstatus, or';

                break;

            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Bkash's SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with Bkash.  '
                 . 'If this problem persists,';
        }
        $msg .= ' let us know at support@bkash.com.';
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\Bkash\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= Bkash::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['bkash-should-retry'])) {
            if ('false' === $rheaders['bkash-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['bkash-should-retry']) {
                return true;
            }
        }
        ...
        //
        // Note that we expect the bkash-should-retry header to be false
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            Bkash::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            Bkash::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(Bkash::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= Bkash::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Bkash use.

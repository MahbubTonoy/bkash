...
abstract class Util
{
    ...
    /**
     * Converts a response from the Bkash API to the corresponding PHP object.
     *
     * @param array $resp the response from the Bkash API
     * @param array $opts
     *
     * @return array|BkashObject
     */
    public static function convertToBkashObject($resp, $opts)
    {
        $types = \Bkash\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                $mapped[] = self::convertToBkashObject($i, $opts);
            }
        ...
        if (null === self::$isMbstringAvailable) {
            ...
            if (!self::$isMbstringAvailable) {
                \trigger_error('It looks like the mbstring extension is not enabled. ' .
                    ...
                    'support@bkash.com if you have any questions.', \E_USER_WARNING);
            }
        }
    ...
    {
        if ($h instanceof \Bkash\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];

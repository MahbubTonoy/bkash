    ...
    public static function parse($options, $strict = false)
    {
        ...
        if (\is_string($options)) {
            if ($strict) {
                ...
                throw new \Bkash\Exception\InvalidArgumentException($message);
            }
        ...
        if (\is_array($options)) {
            ...
            if (\array_key_exists('bkash_account', $options)) {
                $headers['Bkash-Account'] = $options['bkash_account'];
                unset($options['bkash_account']);
            }
            if (\array_key_exists('bkash_version', $options)) {
                $headers['Bkash-Version'] = $options['bkash_version'];
                unset($options['bkash_version']);
            }
        ...
        $message = 'The second argument to Bkash API method calls is an '
           . 'optional per-request apiKey, which must be a string, or '
           . 'per-request options, which must be an array. (HINT: you can set '
           . 'a global apiKey by "Bkash::setApiKey(<apiKey>)")';

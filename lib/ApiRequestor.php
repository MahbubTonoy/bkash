...
class ApiRequestor
{
    ...
    private static $OPTIONS_KEYS = ['api_key', 'idempotency_key', 'bkash_account', 'bkash_version', 'api_base'];
    ...
    public function __construct($apiKey = null, $apiBase = null)
    {
        $this->_apiKey = $apiKey;
        if (!$apiBase) {
            $apiBase = Bkash::$apiBase;
        }
        $this->_apiBase = $apiBase;
    }

    /**
     * Creates a telemetry json blob for use in 'X-Bkash-Client-Telemetry' headers.
     *
     * @static
     *
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        Bkash::getLogger()->error('Serializing telemetry payload failed!');
    ...
    {
        $uaString = 'Bkash/v1 PhpBindings/' . Bkash::VERSION;

        $langVersion = \PHP_VERSION;
        $uname_disabled = static::_isDisabled(\ini_get('disable_functions'), 'php_uname');
        $uname = $uname_disabled ? '(disabled)' : \php_uname();

        $appInfo = Bkash::getAppInfo();
        $ua = [
            'bindings_version' => Bkash::VERSION,
            'lang' => 'php',
        ...
        return [
            'X-Bkash-Client-User-Agent' => \json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _prepareRequest($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Bkash::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
              . '"Bkash::setApiKey(<API-KEY>)".  You can generate API keys from '
              . 'the Bkash web interface.  See https://bkash.com/api for '
              . 'details, or email support@bkash.com if you have any questions.';

            throw new Exception\AuthenticationException($msg);
        }

        // Clients can supply arbitrary additional keys to be included in the
        // X-Bkash-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Bkash::$apiVersion) {
            $defaultHeaders['Bkash-Version'] = Bkash::$apiVersion;
        }

        if (Bkash::$accountId) {
            $defaultHeaders['Bkash-Account'] = Bkash::$accountId;
        }

        if (Bkash::$enableTelemetry && null !== self::$requestTelemetry) {
            $defaultHeaders['X-Bkash-Client-Telemetry'] = self::_telemetryJson(self::$requestTelemetry);
        }

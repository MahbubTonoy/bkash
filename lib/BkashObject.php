 ...
 * Class BkashObject.
 */
class BkashObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToBkashObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "Bkash Notice: Undefined property of {$class} instance: {$k}. "
                    . "HINT: The {$k} attribute was set in the past, however. "
                    . 'It was then wiped when refreshing the object '
                    . "with the result returned by Bkash's API, "
                    . 'probably as a result of a save(). The attributes currently '
                    . "available on this object are: {$attrs}";
            Bkash::getLogger()->error($message);

            return $nullval;
        }
        $class = static::class;
        Bkash::getLogger()->error("Bkash Notice: Undefined property of {$class} instance: {$k}");
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof BkashObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a BkashObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = BkashObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToBkashObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a BkashObject. A BkashObject may contain modified
            //      values within in that its parent BkashObject doesn't know about.
            //
            $original = \array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof BkashObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToBkashObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof BkashObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Bkash object.
     *
     * @return string the JSON representation of the Bkash object
     */
    ...
    /**
     * Sets all keys within the BkashObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any BkashObjects contained as values or
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof BkashObject) {
            $values = $obj->_values;
        } else {
            throw new Exception\InvalidArgumentException(
    ...
    /**
     * @return null|ApiResponse The last response from the Bkash API
     */
    ...
    /**
     * Sets the last response from the Bkash API.
     *
